...
4. 解析原始字节 → 字节序转换 → 位提取 → 数据类型转换 → 工程值计算
5. 将工程值写入线程安全的通用数据模型 (UDM)
...
```
config.toml ──→ LoadDeviceConfig() ──→ DeviceConfig
points.csv  ──→ LoadPointsFromCSV() ──→ []PointConfig
                     │
              ...
              map[string][]ReadChunk   ← 按 RegType 分组，同组内按地址排序，连续地址合并
              {
               ...
               scanOnce()
                   ...
                   │         │  7. 按 DataType 转换 + Scale*值 + Offset → finalVal
                   │         └───────────────────────────────────────────────────┘
                   │              │
                   │              ▼
...
| 6 | Scale | float64 | 缩放系数 |
| 7 | Offset | float64 | 偏移量 |
| 8 | Writeable | bool | "true" 或 "false"（不区分大小写） |
...
10. Scale 无法解析为 float64 → 跳过
11. Offset 无法解析为 float64 → 跳过
12. TagName 重复 → 跳过
...
**步骤 6：按 DataType 转换为工程值**

```
Bool:    finalVal = (extracted == 1)                          → bool
Int16:   effectiveLen==16 → float64(int16(extracted))*Scale + Offset   // 有符号
         effectiveLen<16  → float64(extracted)*Scale + Offset          // 无符号
UInt16:  finalVal = float64(extracted)*Scale + Offset
Int32:   effectiveLen==32 → float64(int32(extracted))*Scale + Offset   // 有符号
         effectiveLen<32  → float64(extracted)*Scale + Offset          // 无符号
...
- **数据结构：** `map[string]interface{}`，key = TagName，value = 工程值
...
```
1. LoadDeviceConfig(configFile)      → 失败则 fatal
2. NewConnManager(cfg)
3. connMgr.Connect()                  → 失败则 fatal（首次连接必须成功）
4. LoadPointsFromCSV(csvFile)         → 失败则 fatal
5. OptimizeChunks(points)
6. udm.New()
7. NewCollector(connMgr, udm, chunks, cfg)
8. go c.ScanLoop(scanInterval)
9. 监听 SIGINT/SIGTERM → 优雅退出
```
