...
- `func LoadDeviceConfig(filePath string) (*DeviceConfig, error)` MUST 保持兼容旧调用，返回第一个设备配置

### R3.3 校验规则 (MUST)

| 字段 | 规则 |
|------|------|
| devices | MUST 支持 `[[devices]]` 多设备配置；MUST 兼容旧版 `[modbus]` 单设备配置 |
| name | 为空时自动设为 `device-N`；旧版 `[modbus]` 自动设为 `default` |
| enabled | 省略时默认 `true`；为 `false` 时启动流程 MUST 跳过该设备采集 |
| csv | 为空时使用命令行 `-csv` 传入的默认点位文件 |
...
type PointConfig struct {
    ...
    BitLen      int      // 仅寄存器类型有效
    Writeable   bool     // CSV 的 0/1 在运行时边界转换为 bool
}
```
...
Int16:   IF effectiveLen==16 → float64(int16(extracted))   // 有符号
         ELSE               → float64(extracted)           // 无符号
UInt16:  finalVal = float64(extracted)
Int32:   IF effectiveLen==32 → float64(int32(extracted))   // 有符号
         ELSE               → float64(extracted)           // 无符号
...
采集值为设备原始值经过字节序、位提取和数据类型解释后的结果，不执行 Scale/Offset 工程值转换。
...
2. Writeable==0 → 返回 `"tag is read-only"`
...
| `LogAllWithDevice(deviceName string)` | 读锁 | 输出 `[DATA] deviceName.TagName = value` |
...
```
1. devices = LoadDeviceConfigs(configFile, csvFile) → 失败则 log.Fatalf
2. 遍历 devices，enabled=false 的设备 MUST 跳过采集启动
3. deviceCSV = cfg.CSV；为空时使用命令行 csvFile
4. points = LoadPointsFromCSV(deviceCSV)        → 失败则记录错误并继续下一个设备
5. connMgr = NewConnManager(cfg)
6. connMgr.Connect()                            → 失败则记录错误并继续下一个设备
7. chunks = OptimizeChunks(points)
8. udmInst = udm.New()
9. collector = NewCollector(connMgr, udmInst, chunks, cfg)
10. go collector.ScanLoop(scanInterval)
11. 成功启动设备数量为 0 时 MUST log.Fatalf
12. 监听 SIGINT/SIGTERM → log 退出信息
```
