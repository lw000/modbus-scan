"use strict";
window.api={
  async request(path,options={}){const response=await fetch(path,options);let body=null;const type=response.headers.get("content-type")||"";if(type.includes("json"))body=await response.json();if(!response.ok){const error=new Error(body?.error?.message||`HTTP ${response.status}`);error.code=body?.error?.code;error.details=body?.error?.details;throw error}return body?.data},
